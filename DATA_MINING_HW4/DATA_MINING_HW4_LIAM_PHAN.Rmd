...
> Comments: We can see that **Mortgage**, **Securities Account**, **Age** and **Experience** are not statistically significant in this logistic regression. **Income**, **EducationGraduate** and **EducationAdvanced/Professional** with **CD Account** seems to be really significant in this model with a very low p-value. **EducationAdvanced/Professional**, **EducationGraduate** and **CD Account** seems to have the most positive impact in wether or not a individual may contract a Loan or not.
...
> Comments: Averaging Probabilities of predictions give us a very slightly, or even the same result than a Majority Vote Method, the Specificity is slightly better.
...
> Comments: CT is the one with the smallest error rate followed by Average Method and Majority Method.
...
# Top Decile
library(lift)
print(paste("Top decile lift by ordering the data by the predictions, and computing the proportion of positives in the top 10%.","Here the top decile is",TopDecileLift(CT_Predictions_Prob[,2], EB_Validation_Pure$`Competitive?`)))
...
# Top Decile
library(lift)
print(paste("Top decile lift by ordering the data by the predictions, and computing the proportion of positives in the top 10%.","Here the top decile is",TopDecileLift(Prediction_Boost$prob[,2], EB_Validation_Pure$`Competitive?`)))
...
# Top Decile
library(lift)
print(paste("Top decile lift by ordering the data by the predictions, and computing the proportion of positives in the top 10%.","Here the top decile is",TopDecileLift(Prediction_Bagging$prob[,2], EB_Validation_Pure$`Competitive?`)))
...
# Create the Function for Confusion Matrix
draw_confusion_matrix_RF <- function(cm2) {
...
# Top Decile
library(lift)
print(paste("Top decile lift by ordering the data by the predictions, and computing the proportion of positives in the top 10%.","Here the top decile is",TopDecileLift(Prediction_RF_Prob[,2], EB_Validation_Pure$`Competitive?`)))
...
> Comments: Single Linkage tend to overfit some cereals to the red cluster while Complete linkage give a bit more say to the classification to other clusters. 

### Centroid for Single Linkage with 5 Clusters
...
### Centroid for Complete Linkage with 5 Clusters
...
### Find Cluster of "healthy cereals"
